...
int float_to_address(float in, int precision, int mantissa_size, int min_exponent){
    ...
    if (current_exponent < (127 + min_exponent)) {
        return -1;
    } else if (current_exponent > ((127 + min_exponent) + (1 << (precision - mantissa_size)) - 1)) {
        return ((1 << precision) - 1);
    }

    return ((current_exponent - (127 + min_exponent)) << mantissa_size) + (in_binary >> (FLOAT_MANTISSA_BITS - mantissa_size) & ((1 << mantissa_size) - 1));
}

// Helper Function
float address_to_float(unsigned int in, int precision, int mantissa_size, int min_exponent){
    int exponent = (in >> mantissa_size) & ((1 << (precision - mantissa_size)) - 1);

    if (in == -1) {
        return 0.0f;
    }

    unsigned int ret = (((127 + min_exponent) + exponent) << FLOAT_MANTISSA_BITS) + ((in & ((1 << mantissa_size) - 1)) << (FLOAT_MANTISSA_BITS - mantissa_size));
    return * (  float  * ) &ret;
}
...
float float_to_diff(float x, unsigned int in, int precision, int mantissa_size, int min_exponent){
    ...
    if (in == -1) {
        return ldexpf(x, -min_exponent);
    }

    if (exponent >= ((1 << (precision - mantissa_size)) - 1)) {
        return 0.0f;
    }
...
#define SIN_COS_TAN_MIN_EXPONENT -7 // This needs to match on CPU and DPU side!
